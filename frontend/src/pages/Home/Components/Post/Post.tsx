...
import {
  IUser,
  useAuthentication,
} from "../../../../context/AuthenticationContextProvider";
...
function Post({ post, setPosts }: PostProps) {
  ...
  const like = async () => {
    ...
    try {
      const response = await fetch(
        import.meta.env.VITE_API_URL + "/api/v1/posts/" + post.id + "/likes",
        {
          method: "PUT",
          headers: {
      ...
      if (error instanceof Error) {
        ...
        console.log("Ops! an error occured. please try again later");
      }
    }
  };
  ...
  const deleteComment = async (id: number) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/v1/comments/delete/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const editComment = async (id: number, content: string) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/v1/comments/update/${id}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const postComment = async (e: FormEvent<HTMLElement>) => {
    e.preventDefault();
    if (!content) {
      return;
    }
  ...
  const deletePost = async (id: number) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/v1/posts/delete/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const editPost = async (content: string, picture: string) => {
    const response = await fetch(
      `${import.meta.env.VITE_API_URL}/api/v1/posts/update/${post.id}`,
      {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
